...
export const PayrollPage: React.FC = () => {
  ...
  const handleExcelAndEmail = async () => {
    ...
    // CRITICAL: Ensure we rely on the current state variable 'viewMode'
    const isMonthlyMode = viewMode === 'monthly';
    const startMonth = isMonthlyMode ? focusedMonth : 0;
    const endMonth = isMonthlyMode ? focusedMonth : 11;
                    ...
                    >
                        {isExporting ? (
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <span>{viewMode === 'monthly' ? `${focusedMonth + 1}월분 전송` : "1년 전체 전송"}</span>
                            </div>
                        )}
                    </button>
                </div>
             ...
             <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[70] flex items-center justify-center px-4">
                <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl animate-fade-in-up">
                    ...
                    <h3 className="text-xl font-black text-center text-slate-800 mb-2">
                        {viewMode === 'monthly' ? `${focusedMonth + 1}월분 파일 저장 완료!` : "전체 파일 저장 완료!"}
                    </h3>
